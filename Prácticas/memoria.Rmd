...
El dataset escogido para realizar las prácticas de esta asignatura es el ***UCI ML Drug Review dataset*** [1]. El objetivo de este conjunto de datos consiste en recopilar información acerca de la efectividad de los fármacos recetados para las enfermedades diagnosticadas. Para ello se recopilan hasta siete campos, cuya naturaleza puede ser numérica o textual. Los campos más relevantes son *drugName*, en el que se encuentran los medicamentos recetados, *condition* que recopila las enfermedades, *review* en el que se almacenan las críticas de los pacientes con respecto a sus respectivas dolencias y tratamientos, así como *rating* que es un valor numérico entre 0 y 10 que el paciente le asigna a un tratamiento según su efectividad.

Otros campos menos relevantes son *uniqueID* con el que se representa el identificador único para cada una de las críticas de los pacientes así como *date*, que hace referencia a la fecha de publicación de las mismas. Por último, asociado también a las *reviews*, disponemos del campo *usefulCount* con el cual otros pacientes pueden apoyar y verificar la utilidad de las críticas de sus compañeros de hospital.

# Problema a resolver

<<<<<REDACTARLO>>>>>>>>
1) Analizar los sentimientos de las críticas para comprobar si son quejas.
2) Comprobar las relaciones entre los términos y cuáles son los más frecuentes.
3) Predecir si un fármaco es efectivo o no en función de su puntuación de 0 a 10.
...
title("Reglas de asociación con soporte=0.1 y confianza=0.8")
```
...
reglas_asoc<-sort(reglas_asoc, decreasing=TRUE, by="confidence")
plot(reglas_asoc, method="graph")
title("Reglas de asociación con soporte=0.01 y confianza=0.8")
```
...
reglas_asoc<-sort(reglas_asoc, decreasing=TRUE, by="confidence")
plot(reglas_asoc, method="graph")
title("Reglas de asociación con soporte=0.1 y confianza=0.4")
```
...
CAMBIAR A 
1) Probamos con la librería kknn pero hay muy pocos ejemplos y nos da un 99% de error.
2) Ejemplos con la librería class y knn y nos da un 34.86667% de aciertos con k=1


Comenzamos esta sección probando la librería *kknn* vista en clase para entrenar un clasificador utilizando la técnica que nos ocupa en cuestión. Para ello hemos aplicado el mismo procedimiento que en la técnica anterior, con la única particularidad de modificar la función encargada de decidir si un término es considerado o no en función de su número de apariciones. Y es que para este algoritmo hemos tenido que convertir la salida que proporciona dicha función en un factor para que pueda trabajar con ella. Finalmente se transforma la matriz de términos obtenida a un *dataframe*, añadiéndole la columna de clasificación de las enfermedades (de 1 a 10) y entrenamos el clasificador.
...
# test_reviews_matriz<-DocumentTermMatrix(test_reviews_corpus)
# # Nos quedamos con los términos más frecuentes (con un mínimo número de aparciciones de 5)
# terminos_frecuentes<-findFreqTerms(train_reviews_matriz, 1)
# # Obtenemos de nuevo la matriz con los términos más frecuentes extraidos anteriormente tanto para train como para test.
# train_reviews_frec_matriz<-DocumentTermMatrix(train_reviews_corpus, control=list(dictionary=terminos_frecuentes))
# test_reviews_frec_matriz<-DocumentTermMatrix(test_reviews_corpus, control=list(dictionary=terminos_frecuentes))
# # Función que decide si tener en cuenta o no el término en función de su número de apariciones.
# frecuencia_binaria<-function(x) { y<-ifelse(x > 0, "Yes", "No") }
# # Obtenemos las matrices de términos tanto de train como de test.
# train_reviews_matriz_bin2<-apply(train_reviews_frec_matriz, 2, frecuencia_binaria)
# test_reviews_matriz_bin2<-apply(test_reviews_frec_matriz, 2, frecuencia_binaria)
...
mat.df <- as.data.frame(data.matrix(train_reviews_matriz), stringsAsfactors = FALSE)
# Column bind category (known classification)
mat.df <- cbind(mat.df, trainNCondition, row.names = NULL)
colnames(mat.df)[ncol(mat.df)] <- "trainNCondition"
# Classifier variable based on the personality typ
cl <- mat.df[,"trainNCondition"]
# Create model data and remove “type”
modeldata <- mat.df[,!colnames(mat.df) %in% "trainNCondition"]
...
######### 99% / con train_reviews_matriz_bin sale 89%
# # Importamos la librería del algoritmo KNN.
# library(kknn)
# # Convertimos la matriz a un dataframe y le añadimos la columna de clasificación de las enfermedades.
# df<-as.data.frame(data.matrix(train_reviews_matriz), stringsAsfactors = FALSE)
# df<-cbind(df, trainNCondition)
# df_test<-as.data.frame(data.matrix(train_reviews_matriz), stringsAsfactors = FALSE)
# df_test<-cbind(df_test, testNCondition)
# # Entrenamos el modelo.
# #knn<-kknn(df$trainNCondition~., df, df_test, na.action=na.omit(), k=5)
# knn<-train.kknn(df$trainNCondition~., df, kmax=5)
# # Obtenemos las predicciones del modelo y su tasa de error sobre test.
# prediccs<-fitted(knn)
# matriz_confusion<-table(prediccs, df_test$testNCondition)
# dd<-diag(matriz_confusion)
# accuracy<-(sum(dd)/nrow(df_test))*100
# test_error<-100-accuracy
# cat("\nError en test:",test_error,"%")
```

Intentamos mejorar el clasificador haciendo uso de la librería *class* [13], en la cual se encuentra la función *knn* especializada en entrenar modelos aplicando la técnica que nos ocupa. Cabe destacar, además, la cantidad de tutoriales existentes, tales como [14], en los que se muestra cómo ejecutar esta función pasándole como datos de entrada texto con el objetivo de predecir una variable categórica. No obstante, tras invertir muchas horas no conseguimos que terminase de entrenar el modelo, simplemente tras comenzar el entrenamiento se bloqueaba RStudio y no hubo forma de obtener un predictor como resultado.
Del mismo modo también probamos con la función *train* ubicada en la librería *caret* [15], la cual también cuenta con diversas técnicas con las que entrenar un clasificador, entre ellas KNN. Y tras realizar el mismo proceso que en sendos casos anteriores el resultado fue el mismo, RStudio se quedaba inoperativo y nunca llegamos a obtener un clasificador entrenado de esta forma. 
...
```{r}
# tm<-DocumentTermMatrix(train_reviews_corpus)
# # Transform dtm to matrix and then back into a data frame for modeling
# mat.df <- as.data.frame(data.matrix(tm), stringsAsfactors = FALSE)
# # Column bind category (known classification)
# mat.df <- cbind(mat.df, mini_training_dataset$rating, row.names = NULL)
# colnames(mat.df)[ncol(mat.df)] <- "rating"
# rg<-lm(mat.df$rating~., mat.df)
```
...
14. Rohit Katti, *Naive Bayes Classification for Sentiment Analysis of Movie Reviews*, 30/04/2016,  https://rpubs.com/cen0te/naivebayes-sentimentpolarity
15. Manual del paquete *caret*, https://cran.r-project.org/web/packages/caret/caret.pdf