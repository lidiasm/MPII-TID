...
train_reviews_corpus<-Corpus(train_reviews_corpus)
...
```{r preprocesamiento_textos, warning=false}
...
<<<<<<<<<<<<<<EXPLICAR QUE SE OBTIENEN LOS TÉRMINOS YA SEPARADOS DESDE EL CORPUS PREPROCESADO CON LA FUNCIÓN TEXT_TOKENS>>>>>>>>>>>>>>>>>>>>>>>>>>
<<<<<<<<<<<<<<<DIBUJAR LOS RESULTADOS>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
<<<<<<<<<<<<<PROBAR VARIANDO EL SOPORTE Y LA CONFIANZA>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
El primer paso que procedemos a realizar consiste en separar los términos de las críticas de modo que obtengamos las palabras individuales y separadas del resto. De este modo, a continuación podremos componer la **base de datos transaccional** en la que cada elemento se corresponde con una única palabra. Una vez dispongamos de los datos preparados podremos obtener las reglas de asociación mediante el paquete ***aRules***. Para ello utilizaremos el algoritmo *apriori*, el cual extrae los conjuntos de palabras más frecuentes y los convierte en reglas de asociación. Como parámetros recibe la base de datos con las transacciones además de argumentos de configuración tales como el soporte y la confianza. 
```{r asocacion, warning=false}
...
itemFrequencyPlot(reviews_bd_transaccional, topN=10)
# Obtenemos las reglas de asociación.
reglas_asoc<-apriori(reviews_bd_transaccional, parameter = list(support = 0.001, confidence = 0.8, target="rules", minlen=2), control = list(memopt = TRUE, load = FALSE))
reglas_asoc<-sort(reglas_asoc, decreasing=TRUE, by="confidence")
inspect(head(reglas_asoc,100))
```
