...
```{r}
library(kknn)
################### MALOS RESULTADOS, MEJORES CON ratingLabel~rating
df<-as.data.frame(train_reviews_matriz_bin, stringsAsfactors = FALSE)
df<-cbind(df, trainNCondition)
df_test<-as.data.frame(test_reviews_matriz_bin, stringsAsfactors = FALSE)
df_test<-cbind(df_test, testNCondition)
knn<-kknn(df$trainNCondition~., df, df_test, k=5)
summary(knn)
fit=fitted(knn)
tt=table(fit, df_test$testNCondition)
tt
dd=diag(tt)
bien=sum(dd)
bien_clasificados=(bien/nrow(df_test))*100
...
13. *Understanding Factors*, https://swcarpentry.github.io/r-novice-inflammation/12-supp-factors/index.html