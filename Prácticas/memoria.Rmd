...
recuento_etiquetas<-summary(training_dataset$condition)
...
n_muestras_max = 500
# Primera enfermedad
mini_training_dataset <- head(subset(training_dataset, training_dataset$condition==etiquetas[1]), n_muestras_max)
# Resto de enfermedades
for (i in c(2:n_enfermedades_max)) {
  mini_training_dataset <- rbind(mini_training_dataset, head(subset(training_dataset,
    training_dataset$condition==etiquetas[i]), n_muestras_max))
}
...
Como podemos comprobar en la gráfica anterior, en este dataset reducido ya no hay clases mayoritarias ni minoritarias, si no que todas disponen del mismo número de muestras representativas. Por lo tanto, hemos conseguido obtener un conjunto de datos de entrenamiento equilibrado con 5.000 muestras.
