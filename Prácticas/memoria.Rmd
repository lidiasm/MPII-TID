...
library(wordcloud2)
library(knitr)
# train_reviews_dataframe<-as.data.frame(as.matrix(train_reviews_matriz))
# colnames(train_reviews_dataframe)<-make.names(colnames(train_reviews_dataframe))
# frecuencia_terminos<-colSums(train_reviews_dataframe)
# frecuencia_terminos<-frecuencia_terminos[order(frecuencia_terminos, decreasing = T)]
#wordcloud(freq = as.vector(frecuencia_terminos), words = names(frecuencia_terminos),random.order = FALSE, colors = brewer.pal(8, 'Dark2'),max.words=100)
...
# Convertimos la colección de documentos en una matriz para operar con ella.
...
```
Tal y como podemos apreciar algunos de los términos más usados en las críticas de los pacientes son *take*, *day*, *feel*, *start*, entre otros. No obstante, si ordenásemos los términos en función de su semántica nos sería de mayor utilidad puesto que nos aportaría una idea acerca de si la mayoría de las palabras contenidas en las *reviews* de los pacientes hacen referencia a una crítica positiva o negativa. 

```{r nube_palabras_sentimientos}
# library(tidyverse)
# library(readr)
# library(RColorBrewer)
# library(tidytext)
# # Convertimos el corpus a una matriz.
# drugstext<-unnest_tokens(mini_training_dataset, word, items)
...
De las opiniones de los pacientes
