...
library(class)
train_reviews_matriz<-DocumentTermMatrix(train_reviews_corpus)
# Transformamos los términos a una matriz y luego a un dataframe para poder trabajar más fácilmente.
train_df<-as.data.frame(data.matrix(train_reviews_matriz), stringsAsfactors = FALSE)
# Añadimos la columna de clasificación de las enfermedades y le añadimos su correspondiente nombre
train_df<-cbind(train_df, trainNCondition, row.names = NULL)
colnames(train_df)[ncol(train_df)]<-"trainNCondition"
...
Tal y como se puede comprobar el clasificador entrenado, considerando los cinco vecinos más cercanos, dispone de una tasa de error sobr el conjunto de prueba del **63,4%**. Si bien ha conseguido mejorar un poco con respecto al resultado anterior con k=1, podemos confirmar que esta técnica no nos aporta un clasificador capaz de predecir la enfermedad en función de las *reviews* de los pacientes, por lo que no es el mejor algoritmo para aplicarlo a nuestros datos. Si analizamos la matriz de confusión, podemos observar que la mayoría de errores se concentran en torno a las etiquetas 3, 4, 6, 7, 8. Esto significia que el clasificador no es capaz de distinguir estas enfermedades de forma correcta en base a las *reviews* de los pacientes.
